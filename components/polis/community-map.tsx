...
export default function CommunityMap({compact=false,visibleItems=items.filter(i=>i.event),selected,onSelect,showFriends=true,following=[],participants}:{compact?:boolean;visibleItems?:CivicItem[];selected?:string;onSelect:(item:CivicItem)=>void;showFriends?:boolean;following?:string[];participants?:Record<string,{name:string;initials:string}[]>}){
